...
The Bicep tenant orchestrator embeds modules for tenant hierarchy/association, Entra security groups, management-group governance/RBAC, subscription resource groups, and resource-group networking/RBAC. Deployment order is input validation, Entra groups, hierarchy, subscription placement, platform and application resources, governance. Group creation is the only step whose client support differs between the portal and the CLI, so it runs before the first Azure mutation and a Graph failure leaves no partial hierarchy. Every supplied subscription, group, owner, and member identifier is checked for GUID shape in the same validation step, because portal users never run the CLI preflight. Stable names make repeat deployments target the same resources.

Two Cloud Adoption Framework recommendations are available as opt-in switches, both off by default so the baseline stays free of tenant-wide setting changes and workspaces: `configureHierarchySettings` sets Sandbox as the default management group for new subscriptions and requires authorization for creating management groups under the tenant root; `enableActivityLogCollection` adds one pay-as-you-go Log Analytics workspace in the management resource group and a diagnostic setting in each placed subscription. Activity-log ingestion and its 90-day retention are free. See the [management group recommendations](https://learn.microsoft.com/azure/cloud-adoption-framework/ready/landing-zone/design-area/resource-org-management-groups#management-group-recommendations) and [activity log export](https://learn.microsoft.com/azure/azure-monitor/platform/activity-log#log-analytics-workspace). No secret, deployment script, paid runtime service, or private registry is required by the deployed template. Compilation restores Microsoft's public Graph extension package; compiled JSON declares MicrosoftGraph provider version 1.0.0 at the root and in Graph modules, including the root import needed for nested authentication. Templates containing Graph resources use ARM languageVersion 2.0 with symbolic resources.
